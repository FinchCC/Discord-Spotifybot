...
namespace MusicBot
{
    public class SpotifyParser
    {
        ...
        public static async Task<string[]> getSongNamesArr(string args)
        {
            ...
            request.AddHeader("x-rapidapi-key", await MISC.Tokens.GetSpotifyToken());
            ...
            names.OrderBy(a => rnd.Next()).ToList();
            try
            {
                File.WriteAllLines(Path.Combine(Directory.GetCurrentDirectory(), "log.txt"), names);
            }
            catch (Exception e)
            {
                Console.WriteLine("Could not log songs, error: " + e.Message);
            }
